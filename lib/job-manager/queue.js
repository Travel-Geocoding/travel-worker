...
const initiatingBulkAPIJobs = require('../job-processors/process-initiating-bulk-api-jobs');
const addDetailToRoute = require('../job-processors/process-route-detail');
...
const Queue = {
  ...
  GEOCODE_ROUTE_DETAIL: new BullQueue('GEOCODE ROUTE DETAIL Queue', REDIS_URL, queueOptions),
};
...
Queue.registerProcessors = function() {
  ...
  Queue.LAUNCH_BULK_API_JOBS.process(function(job) {
    return initiatingBulkAPIJobs(job);
  });

  Queue.GEOCODE_ROUTE_DETAIL.process(function(job) {
    return addDetailToRoute(job);
  });
