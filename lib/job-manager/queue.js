...
Queue.registerProcessors = function() {
  ...
  Queue.INSERTION_MANAGER.process(manageInsertion, { repeat: { every: 1000 } });
};

function manageInsertion() {
  console.log('> cleaning queue and manageing insertion');

  return Queue.INSERT_ROUTE.clean(1000).then(() => {
    if (Queue.INSERT_ROUTE.getWaitingCount() > 10000) {
      return Queue.ROUTES_FOR_LOCATION.pause(false);
    } else {
      return Queue.ROUTES_FOR_LOCATION.resume(false);
    }
  });
}
