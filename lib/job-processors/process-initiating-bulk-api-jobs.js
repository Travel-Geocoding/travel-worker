...
const maxNumberOfCalls = process.env.NUMBER_OF_CALLS_BY_API_KEY || 50;
  ...
  return Promise.resolve(job)
    ...
    .catch((error) => {
      if (error instanceof DomainError.MaxNumberOfCallsExceeded) {
        server.logger().info(`Completed At MaxNumberOfCalls: ${totalCallCount}`);
      } else {
        throw error;
      }
    })
    ...
    .catch((error) => {
      server.logger().error(`Failed launching bulk api job because: ${error}`);
      throw error;
    });
};
