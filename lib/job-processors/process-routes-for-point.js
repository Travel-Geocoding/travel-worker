...
module.exports = function(job) {
  ...
  return Promise.resolve(job)
    .then((job) => {
      server.logger().info(`Start route calculation job`);
    ...
    .then(() => {
      server.logger().info(`Completed route calculation job`);
    })
    .catch((error) => {
      server.logger().error(`Failed route calculation job because: ${error}`);
      throw error;
    });
};
